...
export function FormulárioDeLogin() {
  ...
  const {
    ...
    resolver: zodResolver(SchemaDeLogin),
    defaultValues: {
      email: "",
      password: "",
    },
  ...
  return (
    <form onSubmit={onSubmit} className="flex items-start flex-col w-full gap-y-[10px]">
      ...
      <button
        disabled={isPending}
        className={isPending ? "w-full py-[12px] rounded-lg bg-black/50 flex items-center justify-center" : "w-full py-[12px] rounded-lg bg-black flex items-center justify-center"}
      >
        {isPending ? (
